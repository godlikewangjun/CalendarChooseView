...
public class MonthView extends View {
    ...
    public static final String RANG_TEXT_COLOR = "SELECT_RANGE_TEXT_COLOR";
    ...
    public MonthView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        rangeTextColor =  (int) ATTRS.get(RANG_TEXT_COLOR);
    ...
    private void drawDays(Canvas canvas) {
        ...
        for (int day = 1; day <= dayOfMonth; day++) {
            ...
            if (isFirstDay(day) || isLastDay(day)) {
                ...
                drawSelectPriceText(canvas,"23.5");
            } else {
                if (isPreDay(day)) {
     ...
     * @param selectDayText
     */
    private void drawSelectPriceText(Canvas canvas, String selectDayText) {
        firstSelectPaint.getFontMetrics(fm);
        thirdPaint.getFontMetrics(fm);
        thirdPaint.setColor(selectTextColor);
        float firstAscent = fm.ascent;
        ...
        float daySecondHeight = daySecondDescent - daySecondAscent;
        thirdPaint.getFontMetrics(fm);
        canvas.drawText(selectDayText, dayRang.centerX(), dayRang.top + firstTopMargin + firstHeight + firstHeight+
                        secondTopMargin + daySecondHeight + thirdTopMargin + Math.abs(fm.ascent),
                thirdPaint);
    }
    /**
     ...
     * @param selectDayText
     */
    private void drawPriceText(Canvas canvas,Rect selectRangeRect, String selectDayText) {
        selectPaint.getFontMetrics(fm);
        float firstAscent = fm.ascent;
        float firstDescent = fm.descent;
        float firstHeight = firstDescent - firstAscent;
        dayPaint.getFontMetrics(fm);
        ...
        float daySecondDescent = fm.descent;
        float daySecondHeight = daySecondDescent - daySecondAscent;
        thirdPaint.setColor(rangeTextColor);
        canvas.drawText(selectDayText, selectRangeRect.centerX(),selectRangeRect.top + firstTopMargin + firstHeight + firstHeight+
                secondTopMargin + daySecondHeight + thirdTopMargin + Math.abs(fm.ascent), thirdPaint);
    }
    ...
    private void drawRange(Canvas canvas) {
        ...
        if (isFirstMonth() && isLastMonth()) {
            ...
            for (int day = firstDay + 1; day < lastDay; day++) {
                ...
                drawPriceText(canvas,selectRangeRect,"adasds");
            }
        } else {
            //选择不同月
            //绘制第一次选择的月份 选中范围
